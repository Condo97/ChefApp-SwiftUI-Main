...
struct EasyPantryUpdateView: View {
    ...
    @SectionedFetchRequest<String?, PantryItem> var beforeDaysAgoDateSectionedPantryItems: SectionedFetchResults<String?, PantryItem>
    let onClose: () -> Void
    ...
    NavigationStack {
        EasyPantryUpdateView(
            beforeDaysAgoDateSectionedPantryItems: SectionedFetchRequest<String?, PantryItem>(
                ...
                predicate: NSPredicate(format: "%K <= %@", #keyPath(PantryItem.updateDate), Calendar.current.date(byAdding: .day, value: 0, to: Date())! as NSDate),
                animation: .default),
            onClose: {
